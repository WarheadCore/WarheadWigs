...
local flourishingAncientMarker = mod:AddMarkerOption(true, "npc", 8, 209516, 8) -- Flourishing Ancient
...
function mod:ColossalBlow(args)
	...
	self:Message(args.spellId, "orange", nil, CL.count:format(args.spellName, colossalBlowCount))
...
function mod:VerdantEruption(args)
	...
	self:Message(args.spellId, "cyan", nil, CL.count:format(args.spellName, verdantEruptionCount))
